...
function addNewMedicine(name, packing, generic_name, supplier_name) {
  ...
  xhttp.open("GET", "../ui/php/new_medicine.php?name=" + name + "&packing=" + packing + "&generic_name=" + generic_name + "&suppliers_name=" + supplier_name, false);
  xhttp.send();
}
...
function addMedicineStock(name, batch_id, expiry_date, quantity, mrp, rate, invoice_number) {
  ...
  xhttp.open("GET", "../ui/php/new_purchase.php?action=add_stock&name=" + name + "&batch_id=" + batch_id + "&expiry_date=" + expiry_date + "&quantity=" + quantity + "&mrp=" + mrp + "&rate=" + rate + "&invoice_number=" + invoice_number, false);
  xhttp.send();
}
